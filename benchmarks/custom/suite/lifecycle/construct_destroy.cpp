...
static void bench_move_only_construct_destroy() {
    ...
    BENCH("MoFn ctor/dtor(empty)", c, s);
#else
    BENCH_SOLO("MoFn ctor/dtor(empty)", c);
#endif
}
...
static void bench_function_ref_construct_destroy() {
    ...
    BENCH("FnRef ctor/dtor(empty)", c, s);
#else
    BENCH_SOLO("FnRef ctor/dtor(empty)", c);
#endif
}
