/**
 * @file FunctionRef.tpp
 * @brief FunctionRef template implementation.
 *
 * Contains the implementation of FunctionPro::FunctionRef's out-of-line
 * member functions.
 */
...
FunctionRef<R(Args...)>::FunctionRef(T& callable) noexcept {
    ...
    if constexpr (std::is_pointer_v<DecayT> && std::is_function_v<std::remove_pointer_t<DecayT>>) {
...
template <typename R, typename... Args> R FunctionRef<R(Args...)>::operator()(Args... args) const {
...
template <typename R, typename... Args> FunctionRef<R(Args...)>::operator bool() const noexcept {
