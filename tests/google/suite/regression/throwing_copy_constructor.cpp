...
struct ThrowsOnNthCopy {
    ...
    // constructor, which would otherwise make "moving" this type fall
    // back to the throwing copy constructor. FunctionPro requires stored
    // callables to be nothrow-move-constructible (Function's copy-assign
    // moves through a temporary internally), so provide an explicit,
    // non-throwing move constructor here; only the copy path is meant to
    // simulate failure.
...
// unaffected.
TEST(ThrowingCopyConstructor, AssignThrowKeepsDest) {
    ...
    EXPECT_EQ(dst(), 111); // unaffected by the failed assignment
}

// Verifies a failed copy-construct does not crash, leak, or corrupt the
// source Function it copied from.
TEST(ThrowingCopyConstructor, ConstructThrowKeepsSource) {
    ...
    EXPECT_TRUE(static_cast<bool>(src)); // source untouched by the failed copy
    EXPECT_EQ(src(), 333);
}
