...
TEST(SelfAssign, FunctionSelfCopyAssignIsSafe) {
    ...
    EXPECT_EQ(g(), "hello self-assign");
}

// Verifies self-move-assign on a Function leaves stateful content intact.
TEST(SelfAssign, FunctionSelfMoveAssignIsSafe) {
    Function<std::string()> f = StringCallable{"hello self-assign"};
...
// content intact.
TEST(SelfAssign, MoveOnlySelfMoveAssignIsSafe) {
...
TEST(SelfAssign, FunctionRefSelfCopyAssignIsSafe) {
...
TEST(SelfAssign, FunctionRefSelfMoveAssignIsSafe) {
    auto callable = [] { return 5; };
    FunctionRef<int()> f(callable);
