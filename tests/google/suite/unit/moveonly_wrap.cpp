...
TEST(MoveOnlyWrap, WrapsUniquePtrCapturingCallable) {
...
TEST(MoveOnlyWrap, InvocationReflectsLiveCapturedState) {
    ...
    MoveOnlyFunction<int()> g(std::move(f)); // exercise move() for this binding
    EXPECT_EQ(g(), 4);                       // state (the counter) carries over through the move
}

// Verifies moving a MoveOnlyFunction wrapping a non-copyable callable
// transfers it correctly to the destination.
TEST(MoveOnlyWrap, MoveTransfersNonCopyablePayload) {
...
// stores its address.
TEST(MoveOnlyWrap, RefCanReferenceNonCopyableCallable) {
