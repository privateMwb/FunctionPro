...
static void moved_from_safe_reusable() {
...
static void moved_via_assign_safe() {
    Function<int()> src = [] { return 1; };
    Function<int()> dst;
    dst = std::move(src);
...
// reassignment.
static void moveonly_moved_safe_reusable() {
...
static void run_tests() {
    RUN(moved_from_safe_reusable);
    RUN(moved_via_assign_safe);
    RUN(moveonly_moved_safe_reusable);
}
