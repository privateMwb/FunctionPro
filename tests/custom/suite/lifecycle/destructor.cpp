...
static void destructor_releases_sbo() {
...
// releases that callable's own resources.
static void destructor_releases_heap() {
    auto tracked = std::make_shared<int>(1);
    CHK(tracked.use_count() == 1);
    {
...
static void destructor_empty_safe() {
    // clang-format off
    { Function<int()> f; }
    // clang-format on
...
// releases that callable's own resources.
static void moveonly_destructor_sbo() {
    auto tracked = std::make_shared<int>(1);
    CHK(tracked.use_count() == 1);
    {
...
// releases that callable's own resources.
static void moveonly_destructor_heap() {
    auto tracked = std::make_shared<int>(1);
    CHK(tracked.use_count() == 1);
    {
...
static void moveonly_destructor_empty_safe() {
    // clang-format off
...
static void ref_destructor_no_effect() {
    auto tracked = std::make_shared<int>(1);
...
static void run_tests() {
    RUN(destructor_releases_sbo);
    RUN(destructor_releases_heap);
    RUN(destructor_empty_safe);

    RUN(moveonly_destructor_sbo);
    RUN(moveonly_destructor_heap);
    RUN(moveonly_destructor_empty_safe);

    RUN(ref_destructor_no_effect);
}
