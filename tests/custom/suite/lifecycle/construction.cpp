...
// heap allocation.
static void small_capture_no_allocation() {
    int a = 1, b = 2, c = 3;
...
static void large_capture_allocates_once() {
...
static void destruction_releases_heap() {
    LargePayload payload{};
...
static void moveonly_small_no_alloc() {
    int a = 1, b = 2, c = 3;
    long before = g_allocCount;
...
static void moveonly_large_allocates_once() {
    LargePayload payload{};
    long before = g_allocCount;
    bool invokeOk;
    long delta;
...
// address.
static void ref_never_allocates() {
    int a = 1, b = 2, c = 3;
    LargePayload payload{};
...
static void run_tests() {
    RUN(small_capture_no_allocation);
    RUN(large_capture_allocates_once);
    RUN(destruction_releases_heap);

    RUN(moveonly_small_no_alloc);
    RUN(moveonly_large_allocates_once);

    RUN(ref_never_allocates);
}
