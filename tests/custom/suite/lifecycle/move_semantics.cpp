...
static void move_sbo_empties_source() {
...
// transfers correctly and leaves the source empty.
static void move_heap_empties_source() {
...
static void moveonly_sbo_empties_source() {
    int a = kPad, b = kPad, c = kPad;
...
// callable transfers correctly and leaves the source empty.
static void moveonly_heap_empties_source() {
...
static void ref_move_keeps_source() {
...
static void run_tests() {
    RUN(move_sbo_empties_source);
    RUN(move_heap_empties_source);

    RUN(moveonly_sbo_empties_source);
    RUN(moveonly_heap_empties_source);

    RUN(ref_move_keeps_source);
}
